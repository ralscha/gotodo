...
func (o *Todo) GetAppUser() *AppUser {
	if o == nil {
		return nil
	}

	return o.R.GetAppUser()
}
...
var (
	...
	todoColumnsWithoutDefault = []string{"subject", "app_user_id"}
	todoColumnsWithDefault    = []string{"id", "description"}
...
func (todoL) LoadAppUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTodo any, mods queries.Applicator) error {
	var slice []*Todo
	var object *Todo
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(todoPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert todo, could not build conflict column list")
			}

			conflict = make([]string, len(todoPrimaryKeyColumns))
			copy(conflict, todoPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"todo\"", updateOnConflict, ret, update, conflict, insert, opts...)
